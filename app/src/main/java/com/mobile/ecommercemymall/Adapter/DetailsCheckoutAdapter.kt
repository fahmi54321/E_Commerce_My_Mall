...
class DetailsCheckoutAdapter(
    var data:List<ResultItemProducts?>?,
    var itemClick:onClickListener
):RecyclerView.Adapter<DetailsCheckoutAdapter.ProductMainHolder>() {
    ...
    class ProductMainHolder(itemView:View):RecyclerView.ViewHolder(itemView) {
        ...
        var gambar = itemView.findViewById<ImageView>(R.id.gambar)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ProductMainHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.item_details_checkout, parent, false)
